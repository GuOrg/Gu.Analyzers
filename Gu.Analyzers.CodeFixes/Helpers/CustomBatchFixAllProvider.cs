        ...
        public async virtual Task AddDocumentFixesAsync(Document document, ImmutableArray<Diagnostic> diagnostics, Action<CodeAction> addFix, FixAllContext fixAllContext)
        {
            ...
            for (var i = 0; i < diagnostics.Length; i++)
            {
                ...
                fixerTasks[i] = Task.Run(async () =>
                {
                    ...
                    var task = fixAllContext.CodeFixProvider.RegisterCodeFixesAsync(context) ?? FinishedTasks.Task;
        ...
        public virtual string GetFixAllTitle(FixAllContext fixAllContext)
        {
            ...
            switch (fixAllContext.Scope)
            {
                case FixAllScope.Custom:
                    return $"Fix all occurences of {diagnosticId}";

                case FixAllScope.Document:
                    var document = fixAllContext.Document;
                    return $"Fix all occurences of {diagnosticId} in document {document.Name}";

                case FixAllScope.Project:
                    var project = fixAllContext.Project;
                    return $"Fix all occurences of {diagnosticId} in project {project.Name}";

                case FixAllScope.Solution:
                    return $"Fix all occurences of {diagnosticId} in solution";
